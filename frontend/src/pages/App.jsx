...
import TestAccounts from './TestAccounts.jsx';
...
export default function App() {
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 text-gray-900'>
      <header className='bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-50 border-b border-gray-200'>
        <div className='max-w-7xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center'>
          <div className='flex items-center gap-2 cursor-pointer' onClick={() => setView('home')}>
            ...
            <h1 className='text-lg sm:text-xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent'>ChatVerse AI</h1>
          </div>
          ...
          <nav className='hidden md:flex items-center gap-4 relative'>
            ...
            {!authed ? (
              ...
              <div className='relative'>
                <button onClick={() => setMenuOpen(v => !v)} className='px-3 py-2 rounded-lg font-medium transition-all bg-gray-100 hover:bg-gray-200 flex items-center gap-2'>
                  <div className={`w-7 h-7 rounded-full ${user?.isPremium ? 'bg-gradient-to-br from-amber-500 to-orange-500' : 'bg-gradient-to-br from-indigo-500 to-purple-500'} text-white flex items-center justify-center text-sm relative`}>
                  ...
                  <div className='text-left'>
                    <div className='text-sm font-medium text-gray-800'>{user?.email?.split('@')[0] || 'Account'}</div>
                    <div className={`text-xs ${user?.isPremium ? 'text-amber-600' : 'text-gray-500'}`}>
                ...
                {menuOpen && (
                  <div className='absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-gray-200 p-2 z-50'>
                    <div className='px-3 py-3 border-b border-gray-100'>
                      ...
                      <div className={`text-xs mt-1 px-2 py-1 rounded-full inline-block ${user?.isPremium ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600'}`}>
            ...
            <button onClick={() => setView('test-accounts')} className={`px-3 py-2 rounded-lg font-medium transition-all ${view === 'test-accounts' ? 'bg-purple-100 text-purple-700' : 'text-gray-600 hover:bg-gray-100'}`} title="Test Accounts">
              <svg className='w-5 h-5' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
          ...
          {menuOpen && (
            <div className='md:hidden absolute top-full left-0 right-0 bg-white border-b border-gray-200 shadow-lg z-40'>
              <div className='px-4 py-3 space-y-2'>
                  ...
                  <>
                    <div className='px-4 py-3 border-b border-gray-100'>
                      <div className='flex items-center gap-3'>
                        <div className={`w-10 h-10 rounded-full ${user?.isPremium ? 'bg-gradient-to-br from-amber-500 to-orange-500' : 'bg-gradient-to-br from-indigo-500 to-purple-500'} text-white flex items-center justify-center text-lg relative`}>
                          {user?.email?.[0]?.toUpperCase() || 'U'}
                          {user?.isPremium && (
                ...
                <button onClick={() => { setView('test-accounts'); setMenuOpen(false); }} className={`w-full text-left px-4 py-3 rounded-lg font-medium transition-all ${view === 'test-accounts' ? 'bg-purple-100 text-purple-700' : 'text-gray-600 hover:bg-gray-100'} flex items-center gap-2`}>
                  <svg className='w-5 h-5' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
                    <path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z' />
                  </svg>
                  Test Accounts
      ...
      {view === 'admin' && <Admin />}
      {view === 'test-accounts' && <TestAccounts setView={setView} />}
    </div>
  );
}
